angular.module('MetronicApp').controller('NewsController', function($rootScope, $scope, $http, $base64, $timeout, $location, $q, NewsService, ngDialog, toastr, DTOptionsBuilder, DTColumnBuilder, Upload) {
    ...
    $scope.clickToAddNew = function() {
        ...
        ngDialog.openConfirm({
            template: 'views/news/model_add_news.html',
            ...
            controller: ['$scope', 'data', function($scope, data){
                ...
                $scope.save = function() {
                    ...
                    NewsService.create($scope.img, $scope.mItem).then(function(res) {
    ...
    function getItemByID(id) {
        ...
        var item = {};
        NewsService.getNewss().then(function(res) {
    ...
    $scope.clickToUpdate = function(item) {
        ngDialog.openConfirm({
            template: 'views/news/model_update_news.html',
            className: 'ngdialog-theme-default',
            scope: $scope,
            controller: ['$scope', '$filter', 'data', function($scope, $filter, data){
                ...
                //Load Image
                $scope.img = $scope.settings.imgPath + 'news/' + item.img; 

                // Create News
                $scope.save = function() {
                    $scope.mItem.status = $scope.optionStatus.selected.id;
                    NewsService.update($scope.img, $scope.mItem).then(function(res) {
